...
export const ENGAGEMENT_OBJECT_TYPES = ['resource', 'workspace', 'collection', 'user'] as const;
...
export const engagements = sqliteTable('engagements', {
	...
	subjectType: text('subject_type', { enum: ENGAGEMENT_SUBJECT_TYPES }).notNull(),
	subjectId: text('subject_id').notNull(),
	action: text('action', { enum: ENGAGEMENT_ACTIONS }).notNull(),
	objectType: text('object_type', { enum: ENGAGEMENT_OBJECT_TYPES }).notNull(),
...
export const engagementMentions = sqliteTable('engagement_mentions', {
	engagementId: text('engagement_id')
		.primaryKey()
		.references(() => engagements.id),
	mentionedType: text('mentioned_type', { enum: MENTION_TYPES }).notNull(),
...
export const engagementCommentsRelations = relations(engagementComments, ({ one }) => ({
...
export const engagementMentionsRelations = relations(engagementMentions, ({ one }) => ({
	engagement: one(engagements, {
